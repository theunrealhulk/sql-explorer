...
export class SqliteModel {
  ...
  // Search for tables whose name matches `term`. SQLite exposes a single
  // logical database ("main"), so the grouped result has at most one group.
  static async searchTables(
    ...
    try {
      ...
      const tables = rows
        ...
        .map((r) => ({ name: r.name, schemaName: 'main' }));
      return tables.length ? [{ database: 'main', tables }] : [];
    } finally {
      db.close();
    }
  }
